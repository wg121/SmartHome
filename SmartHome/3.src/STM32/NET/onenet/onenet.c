	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)			//成功解包
			{
				...
				//对数据包 req_playload 进行JSON 格式解析
				json = cJSON_Parse(req_payload);
				if (!json) UsartPrintf(USART_DEBUG,"Error before: [%s]\n",cJSON_GetErrorPtr());	//解析出错
				else			//解析成功
				{
					json_value = cJSON_GetObjectItem(json , "target");		//解析 JSON 的键为:target的值
					UsartPrintf(USART_DEBUG,"json_value: [%s]\r\n",json_value->string);				//输出解析后的json的键值
					UsartPrintf(USART_DEBUG,"json_value: [%s]\r\n",json_value->valuestring);	//输出解析后的json的值
